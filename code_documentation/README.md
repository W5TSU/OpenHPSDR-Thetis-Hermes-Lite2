...
| [`diagrams/`](diagrams/) | One architecture diagram (`.svg`) per `CODE_OUTLINE.md` section, linked from its Contents entry — a grouped component/relationship view of that section's files, generated with the `diagram-design` plugin. Orientation aid, not a substitute for the file-role tables. |
